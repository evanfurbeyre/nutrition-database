<html>
  <body>
    <div>
      <fieldset>
        <legend>Filtered by Food: <?php echo $_POST['fFilter']; ?></legend>
        <table>
            <tr>
              <th>id</th>
              <th>Name</th>
        ...
        if(!($stmt = $mysqli->prepare("SELECT d.dId, d.dName, d.dCal, d.dCost, d.dEffort, d.dFat, d.dSatFat, d.dCarb, d.dSug, d.dProt, d.dSod, d.dText FROM food f INNER JOIN dish_food df ON df.fid = f.fId INNER JOIN dish d ON d.dId = df.did WHERE f.fName = ?"))){
          echo "Prepare error: " . $stmt->errno . " " . $stmt->error;
        }
        if(!($stmt->bind_param("s",$_POST['fFilter']))){
